      ...
      try {
        ...
        if (activeUser) {
          try {
            ...
            if (versionId) {
              await AnalysisRepository.saveAnalysis(activeUser.id, versionId, jd, safeResult);
            }
          } catch (dbErr: any) {
...
export const useGeminiAnalyzer = (): UseGeminiAnalyzerReturn => {
  ...
  const analyze = useCallback(
    async (resume: string, jd: string) => {
      ...
      try {
        ...
        const promptText = `Analyze this resume against this job description: Resume: ${resume}, JD: ${jd}`;
            ...
            if (resumes.length > 0) {
              ...
              if (firstResume.versions && firstResume.versions.length > 0) {
                ...
                const newVersion = await ResumeRepository.createVersion(
                  ...
                  "raw_text",
                  resume.length,
                  AnalysisMapper.mapAnalysisResult(response.text).score || 0,
                  resume
                );
                versionId = newVersion.id;
              }
              ...
              const newVersion = await ResumeRepository.createVersion(
                ...
                "raw_text",
                resume.length,
                ResponseParser.normalizeText(response.text).score || 0,
                resume
              );
              versionId = newVersion.id;
            }
  ...
  const rewriteBullet = useCallback(async (originalText: string, contextDescription: string) => {
    const response = await AIService.handleGeminiCall({
        apiKey: process.env.NEXT_PUBLIC_GEMINI_API_KEY || "",
        model: PromptRegistry.getGeminiModel(),
        contents: `Rewrite this bullet point: ${originalText} with context: ${contextDescription}`,
    }, showToast);
  ...
  const analyzeJobMatch = useCallback(async (resumeText: string, jdText: string) => {
    const response = await AIService.handleGeminiCall({
        apiKey: process.env.NEXT_PUBLIC_GEMINI_API_KEY || "",
        model: PromptRegistry.getGeminiModel(),
        contents: `Analyze job match for resume: ${resumeText} and jd: ${jdText}`,
    }, showToast);
  ...
  const chatWithCopilot = useCallback(
    async (
      ...
      const response = await AIService.handleGeminiCall({
        apiKey: process.env.NEXT_PUBLIC_GEMINI_API_KEY || "",
        model: PromptRegistry.getGeminiModel(),
        contents: `Chat context: ${JSON.stringify(payload)}. Question: ${question}`,
      }, showToast);
